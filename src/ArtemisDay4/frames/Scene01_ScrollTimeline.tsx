/**
 * Scene01_ScrollTimeline — Day 4
 * Duration: 150 frames = 5s (SILENT)
 * Shows full day list, scrolls to day 4
 */
...
export const Scene01_ScrollTimeline: React.FC<Props> = ({ currentDay, totalDays, seriesTitle }) => {
  ...
  const sceneFade = interpolate(frame, [130, 149], [1, 0], { extrapolateLeft: 'clamp', extrapolateRight: 'clamp' });
    ...
    <AbsoluteFill style={{ background: COLORS.bg_paper }}>
      <svg style={{ position: 'absolute', inset: 0, opacity: sceneFade }} width={1080} height={1920}>
        ...
        <g opacity={headerEnter}>
          <text x={60} y={90} fontFamily="'Inter', system-ui, sans-serif" fontSize={28} fontWeight={800}
            fill={COLORS.vibrant_red} letterSpacing="0.05em">
            DAY {currentDay}
          </text>
          <text x={60 + 28 * (String(currentDay).length * 0.7 + 4)} y={90}
            fontFamily="'Inter', system-ui, sans-serif" fontSize={28} fontWeight={400} fill={COLORS.cool_silver}>
            / {totalDays}
          </text>
          <rect x={60} y={110} width={960} height={6} rx={3} fill={COLORS.deep_black} opacity={0.08} />
          <rect x={60} y={110} width={progressEnter} height={6} rx={3} fill={COLORS.vibrant_red} opacity={0.7} />
        </g>

        <text x={540} y={175} textAnchor="middle" fontFamily="'Inter', system-ui, sans-serif"
          fontSize={22} fontWeight={500} fill={COLORS.cool_silver} letterSpacing="0.22em"
              ...
              <g key={d.day}>
                {isCurrent && (
                  <rect x={60} y={rowY + 10} width={960} height={ROW_H - 20} rx={8}
                    fill={COLORS.vibrant_red} opacity={0.06} />
                )}
                {isCurrent && (
                  <rect x={60} y={rowY + 30} width={6} height={160} rx={3} fill={COLORS.vibrant_red} />
                )}
                <text x={isCurrent ? 90 : 80} y={rowY + 85}
                  fontFamily="'Inter', system-ui, sans-serif"
                  fontSize={isCurrent ? 52 : 40} fontWeight={isCurrent ? 900 : 600}
                  fill={isCurrent ? COLORS.vibrant_red : COLORS.deep_black}
