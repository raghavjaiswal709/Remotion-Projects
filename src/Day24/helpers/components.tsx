...
export const GlobalDefs: React.FC = () => (
  <defs>
    ...
    {/* ── PREMIUM: Head chrome metallic gradient ── */}
    <linearGradient id="headChrome" x1="0.15" y1="0" x2="0.85" y2="1">
      <stop offset="0%"   stopColor="#EDF0F2"/>
      <stop offset="20%"  stopColor="#D4DCE0"/>
      <stop offset="55%"  stopColor="#B8C4C8"/>
      <stop offset="100%" stopColor="#8A9498"/>
    </linearGradient>
    {/* Visor dark background */}
    <linearGradient id="visorDark" x1="0" y1="0" x2="0.1" y2="1">
      <stop offset="0%"   stopColor="#080E18"/>
      <stop offset="50%"  stopColor="#030810"/>
      <stop offset="100%" stopColor="#06101A"/>
    </linearGradient>
    {/* Body highlight (light from top-left) */}
    <linearGradient id="bodyShine" x1="0.05" y1="0" x2="0.6" y2="1">
      <stop offset="0%"   stopColor="white" stopOpacity="0.18"/>
      <stop offset="35%"  stopColor="white" stopOpacity="0.06"/>
      <stop offset="100%" stopColor="white" stopOpacity="0"/>
    </linearGradient>
    {/* Arm shine highlight */}
    <linearGradient id="armShine" x1="0.1" y1="0" x2="0.9" y2="1">
      <stop offset="0%"   stopColor="white" stopOpacity="0.14"/>
      <stop offset="50%"  stopColor="white" stopOpacity="0.04"/>
      <stop offset="100%" stopColor="white" stopOpacity="0"/>
    </linearGradient>
    {/* Visor radial focal glow */}
    <radialGradient id="visorFocalGlow" cx="50%" cy="50%" r="55%">
      <stop offset="0%"   stopColor="white"   stopOpacity="0.9"/>
      <stop offset="40%"  stopColor="#80F0FF" stopOpacity="0.6"/>
      <stop offset="100%" stopColor="#00E5FF" stopOpacity="0"/>
    </radialGradient>
    {/* Stronger cyan glow for special elements */}
    <filter id="strongCyanGlow" x="-50%" y="-50%" width="200%" height="200%">
      <feGaussianBlur stdDeviation="22" result="blur"/>
      <feFlood floodColor="#00E5FF" floodOpacity="0.9" result="color"/>
      <feComposite in="color" in2="blur" operator="in" result="colorBlur"/>
      <feMerge><feMergeNode in="colorBlur"/><feMergeNode in="SourceGraphic"/></feMerge>
    </filter>
    {/* Visor ambient glow */}
    <filter id="visorGlow" x="-30%" y="-60%" width="160%" height="220%">
      <feGaussianBlur stdDeviation="8" result="blur"/>
      <feFlood floodColor="#00E5FF" floodOpacity="0.7" result="color"/>
      <feComposite in="color" in2="blur" operator="in" result="colorBlur"/>
      <feMerge><feMergeNode in="colorBlur"/><feMergeNode in="SourceGraphic"/></feMerge>
    </filter>
    {/* Face inset panel gradient */}
    <linearGradient id="faceInset" x1="0" y1="0" x2="0" y2="1">
      <stop offset="0%"   stopColor="#0A0E18"/>
      <stop offset="100%" stopColor="#060A12"/>
    </linearGradient>
    {/* Premium caption glass background */}
    <linearGradient id="captionGlassGrad" x1="0" y1="0" x2="0" y2="1">
...
export const AIRobot: React.FC<RobotProps> = ({
  ...
  return (
    <g transform={`translate(${cx},${cy + totalH/2}) scale(${scale}) translate(${-cx},${-(cy + totalH/2)})`}
      ...
      {/* ── Head ──────────────────────────────────────────────────────── */}
      {/* Head cast shadow */}
      <rect x={headLeft + 5} y={headTop + 8} width={headW} height={headH} rx={28}
        fill="#060810" opacity={0.32}/>
      {/* Head main metallic shell */}
      <rect x={headLeft} y={headTop} width={headW} height={headH} rx={28}
        fill="url(#headChrome)" stroke="#606870" strokeWidth={3}/>
      {/* Head left edge highlight (ambient occlusion) */}
      <rect x={headLeft} y={headTop + 8} width={5} height={headH - 16} rx={2.5}
        fill="white" opacity={0.08}/>
      {/* Head top specular highlight */}
      <ellipse cx={headLeft + headW * 0.38} cy={headTop + 7}
        rx={headW * 0.24} ry={6}
        fill="white" opacity={0.22}/>
      {/* Head outer panel seam */}
      <rect x={headLeft + 12} y={headTop + 12} width={headW - 24} height={headH - 20} rx={20}
        fill="none" stroke="rgba(80,90,100,0.5)" strokeWidth={1.5}/>

      {/* ── Face inset panel ─────────────────────────────────────────── */}
      <rect x={headLeft + 20} y={headTop + 18} width={headW - 40} height={headH - 26} rx={16}
        fill="url(#faceInset)" opacity={0.92}/>
      {/* Face panel inner edge glow */}
      <rect x={headLeft + 21} y={headTop + 19} width={headW - 42} height={headH - 28} rx={15}
        fill="none" stroke={eyeColor} strokeWidth={1} opacity={coreOpacity * 0.25}/>

      {/* ── VISOR (full-width panoramic eye strip) ───────────────────── */}
      {/* Visor housing recess */}
      <rect x={headLeft + 28} y={headTop + 34} width={headW - 56} height={68} rx={14}
        fill="#030609" stroke="rgba(0,229,255,0.18)" strokeWidth={2}/>
      {/* Visor top highlight edge */}
      <rect x={headLeft + 30} y={headTop + 35} width={headW - 60} height={2} rx={1}
        fill={eyeColor} opacity={coreOpacity * 0.45}/>
      {/* Visor glow fill */}
      <rect x={headLeft + 32} y={headTop + 38} width={headW - 64} height={60} rx={10}
        fill={eyeColor} opacity={eyePulse * 0.82} filter="url(#visorGlow)"/>
      {/* Visor scan-lines */}
      {[0, 1, 2, 3, 4].map(si => (
        <line key={si}
          x1={headLeft + 32} y1={headTop + 44 + si * 11}
          x2={headLeft + headW - 32} y2={headTop + 44 + si * 11}
          stroke="rgba(0,0,0,0.28)" strokeWidth={1.5}/>
      ))}
      {/* Left iris focal point */}
      <ellipse cx={headLeft + headW * 0.31} cy={headTop + 68}
        rx={24} ry={22}
        fill="url(#visorFocalGlow)" opacity={eyePulse * 0.5}/>
      <circle cx={headLeft + headW * 0.31} cy={headTop + 68} r={11}
        fill="white" opacity={eyePulse * 0.8}/>
      <circle cx={headLeft + headW * 0.31 + 5} cy={headTop + 63} r={4}
        fill="white" opacity={0.55}/>
      {/* Right iris focal point */}
      <ellipse cx={headLeft + headW * 0.69} cy={headTop + 68}
        rx={24} ry={22}
        fill="url(#visorFocalGlow)" opacity={eyePulse * 0.5}/>
      <circle cx={headLeft + headW * 0.69} cy={headTop + 68} r={11}
        fill="white" opacity={eyePulse * 0.8}/>
      <circle cx={headLeft + headW * 0.69 + 5} cy={headTop + 63} r={4}
        fill="white" opacity={0.55}/>
      {/* Visor bottom reflection strip */}
      <rect x={headLeft + 34} y={headTop + 92} width={headW - 68} height={4} rx={2}
        fill="white" opacity={eyePulse * 0.12}/>

      {/* ── Mouth / speaker grille ───────────────────────────────────── */}
      {/* Grille housing */}
      <rect x={cx - 68} y={headTop + 118} width={136} height={30} rx={8}
        fill="#060A12" stroke="rgba(0,229,255,0.12)" strokeWidth={1}/>
      {/* Grille bars */}
      {[0, 1, 2, 3, 4, 5].map(i => (
        <rect key={i}
          x={cx - 58 + i * 20} y={headTop + 122}
          width={12} height={22} rx={3}
          fill={variant === 'frozen' ? '#202530' : '#0D1520'}
          stroke="rgba(0,229,255,0.2)" strokeWidth={0.8}/>
      ))}
      {/* Grille glow line */}
      <line x1={cx - 66} y1={headTop + 133} x2={cx + 66} y2={headTop + 133}
        stroke={eyeColor} strokeWidth={1} opacity={coreOpacity * 0.4}/>
      ...
      {/* Body top-left specular shine */}
      <rect x={bodyLeft} y={bodyTop} width={bodyW} height={bodyH} rx={32}
        fill="url(#bodyShine)"/>
      {/* Body left-edge highlight strip */}
      <rect x={bodyLeft + 2} y={bodyTop + 16} width={5} height={bodyH - 32} rx={2.5}
        fill="white" opacity={0.07}/>
      {/* Body top surface highlight */}
      <ellipse cx={bodyLeft + bodyW * 0.36} cy={bodyTop + 8}
        rx={bodyW * 0.22} ry={7}
      ...
      <circle cx={bodyLeft - 34} cy={bodyTop + 180} r={14}
        fill="url(#silverGrad)" stroke={COLORS.deep_black} strokeWidth={3}/>
      <circle cx={bodyLeft - 38} cy={bodyTop + 176} r={5}
      ...
      <circle cx={bodyLeft + bodyW + 34} cy={bodyTop + 180} r={14}
        fill="url(#silverGrad)" stroke={COLORS.deep_black} strokeWidth={3}/>
      <circle cx={bodyLeft + bodyW + 30} cy={bodyTop + 176} r={5}
        fill="white" opacity={0.18}/>
...
export const CaptionBar: React.FC<CaptionProps> = ({
  ...
  return (
    <div style={{
      ...
      padding: '0 60px', opacity, zIndex: 100,
    }}>
      <div style={{
        maxWidth: 960,
        textAlign: 'center',
        ...
        border: '1.5px solid rgba(200, 215, 225, 0.14)',
