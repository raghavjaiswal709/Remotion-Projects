/**
 * Shared components for Day 41 — Instance vs Static Variable
...
export const Caption: React.FC<CaptionProps> = ({ text, keyWords = [], frame, sceneFrom, sceneDuration }) => {
  const localFrame = frame - sceneFrom;
  const opacity = Math.min(1, localFrame / 8) * Math.min(1, (sceneDuration - localFrame) / 8);
  ...
  return (
    <g opacity={opacity}>
      {lines.map((lineWords, lineIdx) => (
        <text key={lineIdx} x={540} y={baseY + lineIdx * lineGap} textAnchor="middle"
          fontFamily={FONT} fontSize={44} fontWeight={800} fill={COLORS.text_caption}>
          {lineWords.map((word, i) => {
            const isKey = lowerKeys.some(k => word.toLowerCase().replace(/[.,!?]/g, '').includes(k));
            return (
...
export const BentoCard: React.FC<BentoCardProps> = ({ x, y, w, h, accent = false, rx = 20, opacity = 1 }) => (
...
// ── Corner accents ────────────────────────────────────────────────────────────
export const CornerAccents: React.FC<{ opacity?: number; color?: string }> = ({ opacity = 0.5, color = COLORS.accent }) => (
  <g opacity={opacity}>
...
// ── Section label ─────────────────────────────────────────────────────────────
export const SectionLabel: React.FC<{ text: string; y?: number; opacity?: number }> = ({ text, y = 160, opacity = 0.7 }) => (
  <text x={60} y={y} fontFamily={FONT} fontSize={28} fontWeight={800}
    fill={COLORS.accent} letterSpacing="0.15em" opacity={opacity}>
    {text.toUpperCase()}
  </text>
);
