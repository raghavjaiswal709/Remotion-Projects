/**
 * Shared components for Day 34 — "What Is Task Decomposition?"
...
export const DarkBackground: React.FC = () => (
  <g>
    ...
    {Array.from({ length: Math.ceil(1920 / 120) + 1 }, (_, i) => (
      <line key={`h${i}`} x1={0} y1={i * 120} x2={1080} y2={i * 120}
        stroke={COLORS.grid_line} strokeWidth={1} />
    ))}
    {Array.from({ length: Math.ceil(1080 / 120) + 1 }, (_, i) => (
      <line key={`v${i}`} x1={i * 120} y1={0} x2={i * 120} y2={1920}
        stroke={COLORS.grid_line} strokeWidth={1} />
    ))}
  </g>
);
...
export const GlobalDefs: React.FC = () => (
  <defs>
    ...
    <marker id="arrowMuted" markerWidth="8" markerHeight="8" refX="6" refY="3" orient="auto">
      <path d="M0,0 L0,6 L8,3 z" fill={COLORS.text_muted} />
    </marker>
  </defs>
);

// ── Caption — FIXED POSITION AT BOTTOM ────────────────────────────────────────
