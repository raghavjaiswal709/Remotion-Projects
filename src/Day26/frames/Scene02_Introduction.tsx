/**
 * Scene02_Introduction.tsx — Day 26: Observations
 *
 * "This is day 26 of learning agent AI from first principles."
 *
 * Black background. Number "26" prominently animated. Text reveals word by
 * word. Floating DNA helix of connected nodes. Particle field. Timeline
 * progress bar at bottom. Heavy SVG animation with 50+ elements.
 *
...
/* ── Words for reveal ────────────────────────────────────────────────── */
const words = 'This is day 26 of learning agent AI from first principles.'.split(' ');
...
export const Scene02_Introduction: React.FC = () => {
  ...
  const bigNumberScale = interpolate(frame, [5, 30], [0.2, 1], {
    easing: ease,
    extrapolateRight: 'clamp',
  });

  const bigNumberOpacity = interpolate(frame, [5, 20], [0, 0.12], {
    easing: ease,
    extrapolateRight: 'clamp',
  });
  ...
  const progressWidth = interpolate(frame, [0, 134], [0, 900], {
    easing: Easing.linear,
    extrapolateRight: 'clamp',
  });

  const textBlockOpacity = interpolate(frame, [10, 25], [0, 1], {
    easing: ease,
    extrapolateRight: 'clamp',
  });
  ...
  const particleFade = interpolate(frame, [0, 15], [0, 1], {
    easing: ease,
    extrapolateRight: 'clamp',
  });

  const helixOpacity = interpolate(frame, [8, 28], [0, 0.7], {
    easing: ease,
    extrapolateRight: 'clamp',
  });

  const dayLabelY = interpolate(frame, [10, 28], [20, 0], {
    easing: ease,
    extrapolateRight: 'clamp',
  });
    ...
    <AbsoluteFill style={{ backgroundColor: COLORS.bg_black }}>
      <svg viewBox="0 0 1080 1920" width="1080" height="1920">
        <defs>
          ...
          <radialGradient id="bgGlow26" cx="50%" cy="45%" r="55%">
        ...
        {/* ── Background glow ─────────────────────────────────────── */}
        <rect x="0" y="0" width="1080" height="1920" fill="url(#bgGlow26)" opacity={fadeIn} />
            ...
            return (
              <g key={`helix-${node.id}`}>
                ...
                <circle
                  ...
                  cy={node.baseY}
                  r={Math.max(2, leftR)}
                  fill={depth > 0 ? COLORS.electric_cyan : COLORS.warm_blue}
          ...
          {words.map((word, i) => {
            ...
            return (
              <text
                ...
                fontSize="36"
                fontWeight="500"
                fill={word === '26' ? COLORS.electric_cyan : COLORS.soft_white}
                opacity={wordOpacity}
              >
                {word}
              </text>
        ...
        {/* ── Decorative brackets around text ─────────────────────── */}
        <rect
          x="100"
          y="880"
          width="880"
          ...
          stroke={COLORS.electric_cyan}
          strokeWidth="1"
          opacity={textBlockOpacity * 0.2}
          strokeDasharray="8,6"
        />

        {/* ── Floating small squares ──────────────────────────────── */}
        {Array.from({ length: 8 }, (_, i) => {
          const sqOp = interpolate(
            frame,
            [15 + i * 5, 30 + i * 5],
            [0, 0.3],
            { extrapolateLeft: 'clamp', extrapolateRight: 'clamp' },
          ...
          return (
            <rect
              ...
              strokeWidth="1"
              opacity={sqOp}
              transform={`rotate(${sqRot}, ${sqX}, ${sqY})`}
            />
          );
        })}
        ...
        <g>
          ...
          />
          {/* Labels */}
          <text x="90" y="1800" fontFamily="monospace" fontSize="58" fill={COLORS.cool_silver} opacity={fadeIn * 0.5}>
            00:00
          </text>
          <text x="920" y="1800" fontFamily="monospace" fontSize="58" fill={COLORS.cool_silver} opacity={fadeIn * 0.5} textAnchor="end">
            FIRST PRINCIPLES
          </text>
        </g>
